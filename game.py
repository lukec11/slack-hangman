...
client = CouchDB(
    couch_user, couch_password, url=couch_url, connect=True, auto_renew=True
)
...
class Game:
    def __init__(self, user, word, attempts, case_sensitive, gp, gp_userfunded):
        ...
        self.game[
            "case_sensitive"
        ] = case_sensitive  # Whether the game is case sensitive
        self.game["gp"] = gp  # Value of game in gp
        self.game[
            "gp_funded"
        ] = gp_userfunded  # Whether the game is being funded by a user
    ...
    def start_game(self):
        ...
        result = slack_client.chat_postMessage(
            channel=channel_name, text=game_msg, as_user=True
        )

        self.game["_id"] = result[
            "ts"
    ...
    def display_word(self):
        ...
        self.slack_client.chat_postMessage(
            channel=self.channel_name, thread_ts=self.game["_id"], text=text
        )
    ...
    def guess(self, letter, user):
        ...
        if self.game["user"].lower() != user.lower() and (
            user not in self.game["players"]
        ):
        ...
        for i in range(len(self.game["word"])):
            ...
            if self.game["word"][i] == letter:
                self.game["template"] = (
                    self.game["template"][:i]
                    + letter
                    + self.game["template"][(i + 1) :]
                )
