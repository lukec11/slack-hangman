...
def message_on(**payload):
    ...
    if data.get("text") and data.get("user"):
        ...
        if data.get("channel")[0] == "D":
            ...
            if (
                ...
                reason = (
                    transaction_raw[3].strip()[5:].replace('"', "")
            ...
            if data.get("user") in game_creation_sessions:
                ...
                if not session.get("word"):  # If no word in session, add that first
                    ...
                    session["case_sensitive"] = (
                        True if data.get("text").lower() == "yes" else False
                    )
                    case_sens_str = (
                        ""
                        if session["case_sensitive"]
                        else "Case sensitivity is *disabled*. "
                    )
                    ...
                    session["gp_userfunded"] = (
                        True if data.get("text") == "yes" else False
                    )
                    game_creation_sessions[data.get("user")] = session
    ...
    if data.get("thread_ts") and (
        BOT_ID != data.get("user")
